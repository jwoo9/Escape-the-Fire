...
import {
  ...
  ScrollView, Dimensions, Vibration, Modal, Platform,
} from 'react-native';
...
import { startScanning, stopScanning, onPositionUpdate, isScanning } from '../../services/bleScanner';
import {
...
export default function StaffMap() {
  ...
  const [useSimulation, setUseSimulation] = useState(Platform.OS === 'web');
  ...
  // BLE scanning: auto-start on physical devices (not web/simulator)
  useEffect(() => {
    if (useSimulation) return;

    const unsub = onPositionUpdate((position) => {
      setUserPosition(position);
      setBleActive(true);
      if (user) {
        updateUserLocation(selectedBuilding, user.uid, {
          x: position.x, y: position.y, accuracy: position.accuracy,
          name: user.email?.split('@')[0] || 'Staff', safe: false,
        });
      }
      const perimeter = checkPerimeter(position.x, position.y, selectedBuilding);
    ...
    startScanning(selectedBuilding);
  ...
  }, [selectedBuilding, useSimulation]);
        ...
        <View style={styles.simSection}>
          ...
          {!useSimulation && (
            <View style={styles.bleStatus}>
              <View style={[styles.bleDot, { backgroundColor: bleActive ? '#238636' : '#9a6700' }]} />
              <Text style={styles.bleStatusText}>
                {bleActive ? 'BLE Beacons Detected — Tracking position' : 'Scanning for BLE beacons...'}
              </Text>
            </View>
          )}
          ...
          <View style={styles.modeToggle}>
            <TouchableOpacity
              style={[styles.modeBtn, !useSimulation && styles.modeBtnActive]}
              onPress={() => setUseSimulation(false)}>
              <Text style={[styles.modeBtnText, !useSimulation && styles.modeBtnTextActive]}>
                📡 BLE Beacons
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.modeBtn, useSimulation && styles.modeBtnActive]}
              onPress={() => { setUseSimulation(true); stopScanning(); setBleActive(false); }}>
              <Text style={[styles.modeBtnText, useSimulation && styles.modeBtnTextActive]}>
                🎮 Simulation
              </Text>
            </TouchableOpacity>
          ...
          {/* Simulation presets (only shown in simulation mode) */}
          {useSimulation && (
            <View>
              <Text style={styles.simHint}>Tap a preset to simulate your position</Text>
...
const styles = StyleSheet.create({
  ...
  bleStatus: {
    ...
    padding: 8, backgroundColor: '#0d1117', borderRadius: 6,
  },
  bleDot: { width: 8, height: 8, borderRadius: 4 },
  bleStatusText: { color: '#8b949e', fontSize: 12 },
  modeToggle: { flexDirection: 'row', gap: 6, marginBottom: 10 },
  modeBtn: {
    flex: 1, paddingVertical: 8, alignItems: 'center', backgroundColor: '#21262d',
    borderRadius: 6, borderWidth: 1, borderColor: '#30363d',
  },
  modeBtnActive: { backgroundColor: '#1f3a5f', borderColor: '#58a6ff' },
  modeBtnText: { color: '#8b949e', fontSize: 12, fontWeight: '600' },
  modeBtnTextActive: { color: '#58a6ff' },
